...
from .Bcn3dUpdateCheckerJob import Bcn3dUpdateCheckerJob

i18n_catalog = i18nCatalog("uranium")


class Bcn3dUpdateChecker(Extension):
    ...
    """
    url = "https://api.github.com/repos/bcn3d/stratos/releases/latest"
    ...
    def checkNewVersion(self, silent = False, display_same_version = True):
        ...
        """
        self._download_url = None
        job = Bcn3dUpdateCheckerJob(silent, display_same_version, self.url, self._onActionTriggered, self._onSetDownloadUrl)
        job.start()
    ...
    def _onActionTriggered(self, message, action):
        ...
        connections, so the callback functions need to be long-lived. The Bcn3dUpdateCheckerJob is short-lived so
